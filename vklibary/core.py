...
def UploadInfo():
    ...
    for ac in database.GetInfo("account"):
        ...
        database.Insert("account_monitor",
                        "'{0}', '{1}', '{2}', '{3}'".format(ac["account_id"], budgect, ac["account_name"],
                                                            ac_l))
        time.sleep(1)
        for cm in camp:
            ...
            adIds = {}
            for ad in resp:
                database.Insert("ads", "'{0}', '{1}', '{2}','{3}', '{4}', '{5}', {6}, {7}".format(ad["id"], ad["name"],
                                                                                                  cm["id"],
                                                                                                  ac["account_id"],
                                                                                                  cm["name"],
                                                                                                  ac["account_name"],
...
def GetAcc():
    ...
    c = 0
    for ac in database.GetInfo('account'):
        inf.update({c:{'acname':ac['name'], 'login': ac['login'],
                       'pass': ac['pass']}})
        c += 1
    return inf
# endregionn
...
def GetCamp(start, end, type, office):
    ...
    ofid = 0
    for cm in database.GetInfo("company", sorting="office_id='{0}'".format(office)):
            ...
            ofid = cm['office_id']
    stats.update({c: {'name': type, 'office': ofid,'type': type, 'join': join, 'spent': spent,
                                  'period': {
                                      '30': stat30,
                                      '7': stat7,
                                      '1': stat1
    ...
    for ac in database.GetInfo("account_monitor", "id='{0}'".format(office)):
        for cm in database.GetInfo("company", sorting="office_id='{0}'".format(ac['id'])):
            if type.lower() in cm['name_camp'].lower():
                ...
                stats.update({c: {'name': cm['name_camp'], 'id': cm['id'], 'type': type, 'join': join, 'spent': spent,
                                  'period': {
                                      '30': stat30,
                                      '7': stat7,
                                      '1': stat1
...
def GetAllAds(start, end, type):
    ...
    for ac in database.GetInfo("account_monitor"):
        ...
        summ = ac['summ']
        stats.update(
            {c: {'summ': summ, 'name': ac['name'], 'idof': ac['id'], 'type': type, 'join': join, 'spent': spent}})
        c += 1
    return stats
